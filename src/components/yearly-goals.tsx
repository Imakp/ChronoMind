...
import {
  HierarchyConnector,
  HierarchyTree,
  ExpandableHierarchyItem,
  SimpleHierarchyItem,
} from "@/components/ui/hierarchy-connector";
import { YearlyGoalsEmpty } from "@/components/empty-states";
...
export function YearlyGoals({ yearId, year, initialGoals }: YearlyGoalsProps) {
  ...
  useEffect(() => {
    const handleNewGoal = () => setIsAddingGoal(true);

    window.addEventListener("trigger-new-goal", handleNewGoal);
    window.addEventListener("trigger-new-yearly-goals", handleNewGoal);

    return () => {
      window.removeEventListener("trigger-new-goal", handleNewGoal);
      window.removeEventListener("trigger-new-yearly-goals", handleNewGoal);
    };
  }, []);
  ...
  const handleCreateGoal = async () => {
    ...
    const tempId = `temp-goal-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
  ...
  const handleCreateTask = async (goalId: string, title: string) => {
    const tempId = `temp-task-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
    ...
    <div className="max-w-7xl mx-auto space-y-12 pb-32 animate-in fade-in duration-500">
      ...
      <AnimatePresence>
        {isAddingGoal && (
          <HierarchicalQuickAdd
            ...
            childPlaceholder="Add a phase or milestone..."
            allowChildren={true}
            childrenLabel="phases"
            maxChildren={5}
            onSubmit={async (title, phases) => {
              ...
              startTransition(async () => {
                try {
                  const result = await createGoal(yearId, title);
                  if (result.success && result.data) {
                    const realGoal = result.data as GoalWithRelations;

                    // Replace temp goal with real goal
                    setGoals((prev) =>
                      prev.map((g) => (g.id === tempId ? realGoal : g))
                    );

                    // If phases were provided, create them
                    if (phases && phases.length > 0) {
                      for (const phase of phases) {
                        await handleCreateTask(realGoal.id, phase);
                      }
                    }
            ...
            isExpanded={isAddingGoal}
            onToggle={setIsAddingGoal}
          />
        )}
      </AnimatePresence>
      ...
      <div className="grid gap-8">
        {goals.length === 0 && !isAddingGoal ? (
          <YearlyGoalsEmpty
            year={year}
            onCreateGoal={() => setIsAddingGoal(true)}
          />
        ) : (
          goals.map((goal, i) => (
            <GoalItem
  ...
  return (
    <div className="space-y-4">
      ...
      <QuickAddForm
        ...
        variant="inline"
        size="sm"
        onSubmit={(title) => onCreateTask(goalId, title)}
      />
    </div>
  );
}
...
function SubTaskList({
  ...
  return (
    <HierarchyTree className="relative">
      {subtasks.map((st, index) => (
        <SimpleHierarchyItem
          ...
          className="group/sub"
        >
          <div className="flex items-center gap-3 py-1.5 text-sm min-h-[32px]">
            <button
      ...
      <SimpleHierarchyItem level={1} isLast>
        <div className="mt-2">
          <QuickAddForm
            ...
            variant="inline"
            size="sm"
            onSubmit={(title) => onCreateSubTask(goalId, taskId, title)}
          />
        </div>
      </SimpleHierarchyItem>
    </HierarchyTree>
